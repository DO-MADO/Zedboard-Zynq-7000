...
- 실시간 파라미터/계수 제어 – REST API로 계수나 필터 파라미터를 갱신하면 C 프로세스 stdin으로 명령을 전달해 즉시 반영하는 메커니즘은 현장 튜닝이 잦은 DSP 시스템에서 큰 강점입니다.

- 엔드투엔드 실시간 파이프라인 – 하드웨어 DSP 연산 결과를 3개 스트림(Stage3/5/9)으로 동시에 전송하고, Python에서 추가 계산 없이 바로 웹소켓으로 브로드캐스트하는 구조는 지연을 최소화한 설계로 경쟁력이 있습니다.

- 데이터 보존/분석 편의 – UI에서 저장을 요청하면 수집된 모든 스테이지 데이터를 타임스탬프 기준으로 병합해 KST 시간대 CSV로 내보내도록 구성되어 있어, 사후 분석이나 규제 대응에 유리합니다.

- 원클릭 재배포 및 재시작 – 파라미터 변화 감지 시 파이프라인을 안전하게 중지하고 새로운 설정으로 재시작하는 자동화 로직은 운영 중 설정 변경을 자주 해야 하는 현장 요건에 잘 맞습니다.
