...
class Processor:
    ...
    def _build_buffers(self, target_rate_hz: float):
        ...
        if out_len < 10:  # 안정적 표시를 위한 최소 길이
            out_len = 10
        self.roll_ch = [deque(maxlen=out_len) for _ in range(self.n_ch)]
    ...
    def update_target_rate(self, target_rate_hz: float):
        self._build_buffers(target_rate_hz)
        self._stash = np.empty((0, self.n_ch), dtype=float)  # 디시메이션 윈도우 재시작
    ...
    def process_block(self, block: np.ndarray):
        ...
        self._stash = self._stash[n_full:]                              # 남김
...
def main():
    ...
    # ===== Plot 준비: Figure1(8채널), Figure2(파생), Figure3(슬라이더) =====
    plt.ion()
    ...
    # Figure 3: 컨트롤(슬라이더)
    fig_ctl = plt.figure(figsize=(11, 3))
    fig_ctl.suptitle("Controls", fontsize=12)
    # 슬라이더 영역 배치
    ax_lpf  = fig_ctl.add_axes([0.08, 0.60, 0.84, 0.20])
    ax_mov  = fig_ctl.add_axes([0.08, 0.30, 0.84, 0.20])
    ax_tr   = fig_ctl.add_axes([0.08, 0.05, 0.84, 0.20])

    # 범위 설정
    lpf_max = max(10.0, 0.49 * fs)  # Nyquist 아래로 제한
    s_lpf = Slider(ax=ax_lpf, label=f"LPF cutoff (Hz)  [fs={fs:.0f}]", valmin=1.0, valmax=lpf_max, valinit=LPF_CUTOFF_HZ, valstep=1.0)
    s_mov = Slider(ax=ax_mov, label="Moving-average N", valmin=1, valmax=256, valinit=MOVING_AVG_N, valstep=1)
    s_tr  = Slider(ax=ax_tr,  label="Target rate (Hz)", valmin=1.0, valmax=min(500.0, fs), valinit=TARGET_RATE_HZ, valstep=1.0)

    def on_lpf(val):
        global LPF_CUTOFF_HZ
        LPF_CUTOFF_HZ = float(val)
        LPF_CUTOFF_HZ = max(1.0, min(LPF_CUTOFF_HZ, 0.49*fs))  # 안전 클램프
    ...
    def on_mov(val):
        global MOVING_AVG_N
        MOVING_AVG_N = int(val)
        ax_der.set_title(f"Derived: {PLOT_SIGNAL}  (MA={MOVING_AVG_N})")
    s_mov.on_changed(on_mov)

    def on_tr(val):
        global TARGET_RATE_HZ
        TARGET_RATE_HZ = float(val)
        TARGET_RATE_HZ = max(1.0, TARGET_RATE_HZ)
        proc.update_target_rate(TARGET_RATE_HZ)   # 롤링버퍼/디시메이션 재구성
        ax_der.set_title(f"Derived: {PLOT_SIGNAL}  (Target={TARGET_RATE_HZ:.0f}Hz)")
    ...
    try:
        while True:
            ...
            # ==== 성능 로그 ====
            # eff_sps: 루프당 처리된 샘플(프레임) 기준 효율 샘플레이트(채널당 아님)
            eff_sps = BLOCK_SAMPLES / ((read_ms + proc_ms) / 1000.0) if (read_ms + proc_ms) > 0 else 0.0
            est_out_hz = fs / proc.decim_ratio  # 기대 출력 레이트(디시메이션 후)
            print(f"\rread {read_ms:6.1f} ms | proc {proc_ms:6.1f} ms | eff ~{eff_sps/1e3:6.1f} kS/s | out≈{est_out_hz:4.1f} Hz | ch={n_ch}", end="")
